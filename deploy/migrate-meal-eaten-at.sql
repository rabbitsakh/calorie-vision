-- Optional user-editable meal time on MealEntry.
...
CALL `_add_column_if_not_exists`('MealEntry', 'eatenAt', 'DATETIME(3) NULL');
DROP PROCEDURE IF EXISTS `_add_column_if_not_exists`;

SELECT 'migrate-meal-eaten-at: ok' AS status;
