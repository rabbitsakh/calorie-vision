/**
 * Centralized Russian copy for mascot companion surfaces (empty diary, push prompt).
 */

export const MASCOT_COPY = {
  emptyDiary: {
    /** Accessible / title attribute on the empty-state mascot. */
    title: "Пустой дневник",
    headline: "Дневник пуст — добавьте первый приём пищи",
    body: "Сфотографируйте тарелку или введите название — прогресс появится сразу.",
  },
  emptyWeight: {
    title: "Нет измерений веса",
    headline: "Добавьте первое измерение",
    body: "Вес нужен для нормы калорий и цели. Укажите текущий вес в форме выше.",
  },
  pushPrompt: {
    title: "Напоминания от талисмана",
    body: "Обед, ужин, серия, итог недели, мягкое возвращение и вечерний чек-ин — без давления. Завтрак, воду и сводку калорий можно включить в профиле.",
    enable: "Включить",
    enabling: "Подключаем…",
    install: "Установить приложение",
    later: "Не сейчас",
  },
  pushIosHint: {
    title: "Напоминания на iPhone",
    fallbackBody:
      "Откройте приложение с иконки на экране «Домой» — из Safari уведомления не приходят.",
    statusHint: "Статус можно проверить в разделе «Профиль».",
    install: "Как установить",
    dismiss: "Понятно",
  },
} as const;

export type MascotCopy = typeof MASCOT_COPY;
