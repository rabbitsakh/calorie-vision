...
- package/label: читай КРУПНЫЙ текст с упаковки буквально. Овсянка/геркулес/хлопья/каша — не суп; не угадывай по форме стаканчика
...
export function buildBarcodeLookupPrompt(barcode: string, webContext?: string): string {
  ...
  const webBlock = webContext?.trim()
    ? `\n\n${webContext.trim()}\n`
    : `\n\nИнтернет-подсказок нет — опирайся на знание штрихкодов: РФ/СНГ (460…), импорт/CN (690–699…), EU (400–440…). Для пива/напитков оцени объём с упаковки (мл≈г) и типичные ккал/100 мл.\n`;
